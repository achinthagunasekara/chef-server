...
sudo LD_LIBRARY_PATH="/opt/omnibus-gcc/embedded/lib64 chef-server-ctl reconfigure --chef-license=accept-no-persist
sleep 120
...
sudo LD_LIBRARY_PATH="/opt/omnibus-gcc/embedded/lib64 chef-server-ctl test -J pedant.xml --all --compliance-proxy-tests
...
if [[ "${OMNIBUS_FIPS_MODE:-false}" == "true" ]]; then
  ...
  echo ""
  sleep 120
  sudo LD_LIBRARY_PATH="/opt/omnibus-gcc/embedded/lib64 chef-server-ctl test -J pedant-fips.xml --smoke
fi
